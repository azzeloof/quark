...
## Problem
We want to have a web form that posts data to a server, but the server in question is on an arbitrary network that we don't own, and cannot forward ports through - or, we don't need a constant stream of data, and want to just poll the data queue for chunks of messages at our leisure without dealing with sockets and callbacks.
